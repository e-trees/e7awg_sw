...
[example/zcu216](../../../examples/zcu216) ディレクトリ以下にあるサンプルスクリプトを参考にしてください．
...
| upl axi rw | UDP/IP 通信用のコア (e7udpip10G) と 波形データ RAM 間のデータの送受信を制御します． |
...
## 3. 波形データ RAM ソフトウェアインタフェース仕様

波形データ RAM は 1 ワード 512 bits (= 64Bytes) のメモリで，ワード単位でアクセス可能です．
波形データ RAM は AWG と一対一に対応しており，合計 16 個あります．
全ての波形データ RAM は専用のメモリ空間にマッピングされており，そのアドレスと波形データ RAM の対応は以下の図の通りです．


![波形データRAMアドレッシング0](./figures/wave_ram_addressing.png)


## 3.1 波形データ RAM アクセスパケットフォーマット
e7awg_hw の波形データ RAM にアクセスするためには，波形データ RAM アクセスパケットを UDP データとして e7awg_hw のポート 16384 に送る必要があります．
波形データ RAM アクセスパケットには，以下の 4 種類があり，(A) と (C) が e7awg_hw に送るパケットで，(B) と (D) がその応答として e7awg_hw から送られるパケットです．

- (A) 波形データ RAM 読み出しパケット
- (B) 波形データ RAM 読み出し応答パケット
- (C) 波形データ RAM 書き込みパケット
- (D) 波形データ RAM 書き込み応答パケット

#### (A) 波形データ RAM 読み出しパケット
このパケットを e7awg_hw に送信すると，アドレス `A` から `A + B - 1` までのワードデータが，波形データ RAM 読み出し応答パケットとして返ってきます．A と B は共に 64 の倍数を指定してください．B の最大値は 4032 です．

![波形データRAM読み出し](./figures/udp_wave_ram_read.png)

#### (B) 波形データ RAM 読み出し応答パケット
波形データ RAM の読み出しに成功した場合，**アドレス** と **バイト数** フィールドには，波形データ RAM 読み出しパケットで指定した値が入っています．**ワードデータ 1 ~ N** には，アドレス `A` から `A + 64N - 1` までのワードデータが順に格納されています．

![波形データRAM読み出し応答](./figures/udp_wave_ram_read_resp.png)

#### (C) 波形データ RAM 書き込みパケット
このパケットを e7awg_hw に送信すると，アドレス `A` から `A + 64N - 1` までのワードに，**ワードデータ 1 ~ N** の値が書き込まれます．A は 64 の倍数を指定してください．N の最大値は 63 です．

![波形データRAM書き込み](./figures/udp_wave_ram_write.png)

#### (D) 波形データ RAM 書き込み応答パケット
波形データ RAM への書き込みに成功した場合，**アドレス** と **バイト数** フィールドには，波形データ RAM 書き込みパケットで指定した値が入っています．

![波形データRAM書き込み応答](./figures/udp_wave_ram_write_resp.png)


## 3.2 波形データ RAM  データレイアウト
各 AWG は，以下の図で示す 波形データ RAM の領域を使用しなければなりません．
それぞれの領域での波形データの並びは，**4.5 波形データ RAM に格納された波形データの並び** を参照してください．

![波形データRAMデータレイアウト](./figures/wave_ram_data_layout.png)
