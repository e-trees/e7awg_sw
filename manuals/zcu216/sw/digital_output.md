...
本資料は，ZCU216 を利用したディジタル出力モジュールの利用者向けマニュアルです．

## 1. システム構成

ディジタル出力モジュールは ZCU216 の FPGA 上に実装されており，そのシステム構成は以下のようになります．
...
ディジタル出力モジュール 0 の出力値は ZCU216 の PMOD 0 の電圧値として出力されます．
...
with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as trasnceiver,
      e7s.DigitalOutCtrl(fpga_ip_addr, e7s.E7AwgHwType.ZCU216) as digital_out_ctrl):

    # FPGA コンフィギュレーション
    e7sz.configure_fpga(trasnceiver, e7s.E7AwgHwType.ZCU216)
...
with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as trasnceiver,
    ...
    # ディジタル出力データの作成
    dout_data_list = e7s.DigitalOutputDataList(e7s.E7AwgHwType.ZCU216)
    (dout_data_list
        .add(0x01, 100)
        .add(0x02, 150)
        .add(0x04, 200))
