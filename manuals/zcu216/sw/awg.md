...
本資料は，ZCU216 を利用した Arbitrary Waveform Generator (以下 AWG) の利用者向けマニュアルです．

## 1. システム構成

AWG は ZCU216 の FPGA 上に実装されており，そのシステム構成は以下のようになります．
...
- サンプリングレート : 9830.4 [Msps]
- I/Q ミキサ : 有効
- インタポレーション : 4倍


AWG から出力される **ユーザ定義波形** は，DAC 内部でサンプル数が 4 倍になる様に補間された後，DAC のサンプリングレートで出力されます．
よって，例えばサンプル数が 512 * n 個 (n = 1, 2, 3, ... ) の **波形パート** は，長さが 208.333 * n [ns] (≒ 4 * 512 * n / 9830.4 * 1000) の波形となって出力されます．
...
### 5.1. RF Data Converter の DAC タイルと DAC タイル ID の対応関係

本 API には RF Data Convnerter の DAC タイルを指定して実行するメソッドがあります．
各タイルに対応する ID は e7awgsw.zcu216 パッケージの DacTile クラスに列挙子として定義されており，その対応関係は以下の通りです．
...
fpga_ip_addr = '10.0.0.16'     # ZCU216 の 10 Gb Ethernet ポートの IP アドレス

# DAC/AWG 制御用オブジェクトを作成する
with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as trasnceiver,
...
# DAC/AWG 制御用オブジェクトを作成する
with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as trasnceiver,
      e7sz.RfdcCtrl(trasnceiver, e7s.E7AwgHwType.ZCU216) as rfdc_ctrl,
      e7s.AwgCtrl(fpga_ip_addr, e7s.E7AwgHwType.ZCU216) as awg_ctrl):
