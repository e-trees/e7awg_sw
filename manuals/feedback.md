...
具体的には，キャプチャユニット N  (=0~9) が受け取ったキャプチャデータから算出された四値化結果は，四値チャネル N に送られます．
...
高速フィードバックコマンドの実行前に，本コマンドで動く全ての AWG に対して四値チャネルと波形パラメータブロックが指定されていなければなりません．
...
下図の AWG スタート時刻はコマンドの`スタート時刻`フィールドで指定した時刻です．
