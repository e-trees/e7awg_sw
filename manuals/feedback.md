...
| 確認時刻 | シーケンサの状態が `RUNNING` になった瞬間を 0 とし，`8 * 確認時刻` [ns] 後に波形出力の終了を確認します．この時刻に波形の出力が終了していなかった場合，コマンドエラーレポートが発行されます． |
| 強制停止フラグ | 1 にすると，指定した AWG の波形出力が `確認時刻` に終了していなかった場合，それらに強制停止命令を送ります．強制停止命令は，波形出力の終了待ちを行わない場合でも送信されます． |
...
コマンドの実行時間はメモリの読み出しタイミングなどにより変わることがありますが，本節では特定の条件での最大値を記します。
...
\begin{array}{ll}
  ...
  不定 & (スタート時刻フィールドのビットが全て 1 ではない場合)\\
\end{array} \\
\right. \\[3ex]
...
\\[3ex]
B &= 2x \\
x &= コマンドで指定した AWG が出力するユーザ定義波形の中で最長のものに含まれるサンプル数
...
キャプチャパラメータ設定コマンドは `設定要素` に指定したキャプチャパラメータによって実行時間が変わります．
キャプチャパラメータはグループごとにパラメータ番号が割り当てられており，実行時間の算出に使用します．
...
\begin{align*}
A &= 21 + x \\
x &= 設定するキャプチャパラメータが属する番号のうち最も小さいもの \\
\end{align*}
$$
-->

![キャプチャパラメータ設定コマンド式](./feedback_figures/cmd_time/cap_param_set_exp.png)

1 つのパラメータ番号に属するパラメータを設定する際，実行時間 B は以下の表に従います．
パラメータ番号 1 に属するパラメータは，設定する個数によらずコマンドの実行時間は一定です．
異なるパラメータ番号のパラメータを設定する場合，個々のパラメータ番号に対応する実行時間の合計を超えないことが保証されます．
...
| ---- | ---- |
| レポート ID | 0x07 |
| AWG ID リスト | 波形出力終了フェンスコマンドで指定した AWG のうち，指定した時刻に波形出力が完了していなかったもののリストです．<br> N bit 目が N 番目の AWG に対応しています．<br> 同コマンドの強制終了フラグと終了待ちフラグは，このフィールドの結果に影響を与えません． <br> 同コマンドが確認時刻より前に中断された場合，このフィールドは 0 となります． |
