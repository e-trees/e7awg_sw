...
|  モジュール  |  機能  |
| ---- | ---- |
| sequencer | フィードバック制御コマンドを順次実行しながら，コマンドの内容に応じて他のモジュールに命令を出します． |
...
具体的には，キャプチャユニット N  (=0~9) が保存したキャプチャデータから算出されたフィードバック値は，フィードバックチャネル N に送られます．
...
計算結果は，計算元のキャプチャデータを受け取ったキャプチャユニットごとに，異なるデータパス (**内部四値チャネル**) に送られ，シーケンサから参照できるようになります．
具体的には，キャプチャユニット N (=0~9) が受け取ったキャプチャデータから算出された四値化結果は，内部四値チャネル N に送られます．
フィードバック制御コマンドでキャプチャユニットが算出した四値化結果を参照する際は，内部四値チャネルを指定します．
四値化結果は，<u>キャプチャユニットの四値化処理が有効になっている場合</u>，キャプチャ区間ごとに 1 つ内部四値チャネルに送られます．
1 つのキャプチャ区間から複数の四値を算出する場合は，そのキャプチャ区間で最初に計算された四値が内部四値チャネルに送られます．
キャプチャ区間および四値化の詳細は，[e7awg_hw ユーザマニュアル](./README.md) の **4.2 キャプチャ区間の定義** および **4.3 キャプチャユニット詳細** を参照してください．

![内部四値チャネル](./feedback_figures/four_cls_val_channel.png)


### 1.5 フィードバックシステム外部から sequencer に入力される信号

| 信号名 | 説明 |
| ---- | ---- |
| 外部四値チャネル | フィードバックシステムの外部から四値を入力するためのポートです．<br>1.4 で示した内部四値チャネルとは別のポートです．<br> 外部四値チャネルの四値も内部四値チャネルと同様に，特定のフィードバックコマンドで参照可能です． |
| 外部 AWG スタートトリガ | 特定のフィードバック制御コマンドは，この信号の立ち上がりを待って AWG をスタートします． |
| start | この信号が立ち上がると `sequencer` がフィードバック制御コマンドの処理を開始します．|
| branch_flag_n | フィードバック制御コマンドの実行順序を制御するためのポートです． |
...
コマンドカウンタは，コマンドの処理が開始されるたびに自動的に 1 ずつ増えますが，3.4 節で説明する `Branch By Flag コマンド` を使うと，コマンドカウンタを任意の値に変更することが可能です．
...
| スタート時刻 | シーケンサの状態が `RUNNING` になった瞬間を 0 とし，`8 * スタート時刻` [ns] 後に AWG をスタートします．<br><br> ただし，**全てのビットが 1 のときは，コマンドの実行と同時に AWG の波形出力準備を行い，全ての AWG の準備が完了すると波形の出力を開始します．** <br> コマンドの開始時に操作対象の AWG の中に動作中のものがある場合, 停止するのを待ってから波形の出力準備と出力を行います．<br> その場合，このコマンドは失敗扱いとなりエラーレポートが発行されます．|
...
※波形パラメータ格納領域は，各 AWG が個別に持つため，格納領域ごとに当該 ID の波形パラメータを本コマンドの実行前に設定しておく必要があります．
...
| フィードバックチャネル ID | 参照するフィードバックチャネルの ID (= 0~9) . |
...
本コマンドで実現可能な AWG およびキャプチャユニットの動作例を以下に示します．
...
AWG と参照する四値チャネルおよびロードされる波形パラメータの組み合わせは，`波形パラメータ選択コマンド` で指定してください．

各キャプチャユニットとそれにスタート信号を与える AWG の対応は，キャプチャ制御レジスタの `トリガー AWG 選択 0 ~ 3`，`AWG トリガマスク` レジスタで指定します．キャプチャ制御レジスタの詳細は，[e7awg_hw ユーザマニュアル](https://github.com/e-trees/e7awg_sw/tree/main/manuals) の **4.4 キャプチャ制御レジスタ一覧** を参照してください．
...
| スタート時刻 | シーケンサの状態が `RUNNING` になった瞬間を 0 とし，`8 * スタート時刻` [ns] 後に AWG をスタートします．<br><br> ただし，**全てのビットが 1 のときは，コマンドの実行と同時に AWG の波形出力準備を行い，<br> 全ての AWG の準備が完了すると波形の出力を開始します．** <br> コマンドの開始時に操作対象の AWG の中に動作中のものがある場合, 停止するのを待ってから波形の出力準備と出力を行います．<br> その場合，このコマンドは失敗扱いとなりエラーレポートが発行されます．|
...
`高速フィードバックコマンド`，`四値付き外部トリガ待ち AWG スタートコマンド`，`条件付きフィードバックコマンド` で AWG が出力する波形の候補を指定するコマンドです．
`高速フィードバックコマンド` では，AWG が 2 回目の波形を出力する前に四値チャネルを参照して，その値に応じて波形パラメータブロックから AWG に波形パラメータをロードします．
`四値付き外部トリガ待ち AWG スタートコマンド` では，AWG が波形を出力する前に四値チャネルを参照して，その値に応じて波形パラメータブロックから AWG に波形パラメータをロードします．
`条件付きフィードバックコマンド` では，AWG が 2 回目の波形を出力する前に，同コマンドで指定されたフィードバックフラグを参照して，その値に応じて波形パラメータブロックから AWG に波形パラメータをロードします．その後，四値チャネルの値とフィードバックフラグの値を比較して，出力波形を 0 データにするかどうかを判断します．このコマンドは，その際の **AWG**，**四値チャネル**，**波形パラメータブロック** の組み合わせを指定します．

`高速フィードバックコマンド`，`四値付き外部トリガ待ち AWG スタートコマンド`，`条件付きフィードバックコマンド` で動作する全ての AWG に対して，事前に本コマンドで四値チャネルと波形パラメータブロックを指定する必要があります．
...
| 四値チャネル ID | `外部トリガフラグ` と合わせて参照する四値チャネルが決まります．<br> このフィールドの値が N のとき，<br>外部トリガフラグが 0 なら，内部四値チャネル N (= 0 ~ 9) が指定されます．<br> 外部トリガフラグが 1 なら，外部四値チャネル N (= 0) が指定されます．|
| ブロック ID (0~3) | AWG に設定する波形パラメータブロックの ID． <br> 四値化結果によって参照されるフィールドが変わり，四値化結果が N (= 0~3) のとき，ブロック ID (N) が参照されます．<br> `条件付きフィードバックコマンド` では，指定されたフィードバックフラグが M (= 0 ~ 3) であった場合，ブロック ID (M) が参照されます．|
| 外部トリガフラグ | `四値チャネル ID` と合わせて，参照する四値チャネルが決まります．|
...
本コマンドは `AWG ID リスト` フィールドで指定された各 AWG に対し

- 波形パラメータの更新準備
- 波形パラメータを決める四値の更新待ち
- 四値に応じた波形パラメータの変更

をした後，他の AWG の波形パラメータの更新と外部 AWG スタートトリガの入力を待ってから AWG を同時にスタートします．
AWG と参照する四値チャネルおよびロードされる波形パラメータの組み合わせは，`波形パラメータ選択コマンド` で指定してください．
`四値` と `外部 AWG スタートトリガ` はどちらが先に入力されても問題ありません．

![四値付き外部トリガ待ちAWGスタートコマンド](./feedback_figures/awg_start_with_ext_trig_and_cls_val_cmd.png)

| フィールド名 | 説明 |
| ---- | ---- |
| コマンド ID | 0xB |
| AWG ID リスト | N bit 目が 1 のとき，AWG N をスタートします．(複数選択可能) |
| タイムアウト時刻 | このコマンドの開始時刻から `8 * タイムアウト時間` [ns] までの間に外部 AWG スタートトリガが入力されなかった場合，このコマンドは失敗となり AWG をスタートせずに終了します． |
| 終了待ちフラグ | 1 にすると，スタートした全ての AWG の波形出力が終了するまでコマンドを終了しません． |
...
1. コマンドで指定した AWG が，指定された時刻に同時に波形を出力します
2. AWG からスタート信号を受け取ったキャプチャユニットがキャプチャを開始します
3. 波形の出力が終了した AWG にフィードバックフラグに指定した四値に対応する波形シーケンスを設定します
4. キャプチャユニットが四値化結果を出力します
5. コマンドで指定した全ての AWG の波形パラメータの設定と四値化結果の算出が完了すると，AWG が同時に 2 回目の波形を出力します．
このとき AWG ごとに，対応する四値チャネルの四値化結果とフィードバックフラグの値を比較し，2 つの値が一致しない AWG は出力波形が 0 データになるように設定します．

![条件付きフィードバックコマンドタイミングチャート](./feedback_figures/conditional_feedback_cmd_timing_chart.png)

1 回目に出力される波形のパラメータは，コマンド開始時に AWG に設定されているものです．
2 回目に出力される波形は，四値化結果とフィードバックフラグによって決まります．
AWG に対応する四値チャネルの四値化結果と，コマンドで指定されたフィードバックフラグの値が一致した場合，
2 回目に出力される波形は，フィードバックフラグの値に対応する波形パラメータとなります．
一致しない場合は 0 データとなります．
0 データの波形の長さは，一致した場合と変わりません．
AWG と参照する四値チャネルおよびロードされる波形パラメータの組み合わせは，`波形パラメータ選択コマンド` で指定してください．

![条件付きフィードバックコマンド](./feedback_figures/conditional_feedback_cmd.png)
...
| スタート時刻 | シーケンサの状態が `RUNNING` になった瞬間を 0 とし，`8 * スタート時刻` [ns] 後に AWG をスタートします．<br><br> ただし，**全てのビットが 1 のときは，コマンドの実行と同時に AWG の波形出力準備を行い，全ての AWG の準備が完了すると波形の出力を開始します．** <br> コマンドの開始時に操作対象の AWG の中に動作中のものがある場合, 停止するのを待ってから波形の出力準備と出力を行います．<br> その場合，このコマンドは失敗扱いとなりエラーレポートが発行されます．|
| 終了待ちフラグ | 1 にすると，AWG ID リストで指定した全ての AWG の 2 回目の波形出力が終了するまでコマンドを終了しません． |
| フィードバックフラグ | AWG から 2 回目に出力される波形を決めるための値です. <br> この値が n のとき `波形パラメータ選択コマンド` で指定した `ブロック ID (n)` の波形パラメータが AWG に設定されます．<br> また，同コマンドで指定した四値チャネルの四値化結果とこのフィールドの値が一致しなかった場合，設定された波形パラメータに関わらず，2 回目に出力される波形は 0 データとなります． |
...
本節のタイムチャートは，AWG 制御レジスタの `波形送信可能ブロック周期` レジスタの値が 1 であると仮定しています．
このレジスタの詳細は [e7awg_hw ユーザマニュアル](https://github.com/e-trees/e7awg_sw/tree/main/manuals) の **3.3 AWG 制御レジスタ一覧** を参照してください．
...
下図の AWG スタート時刻は，コマンドの `スタート時刻` フィールドで指定した時刻です．
...
本項では，1 回目の波形出力に合わせてスタートしたキャプチャユニットが算出した四値を元に，2 回目に出力される波形が決まる場合の実行時間を示します．
高速フィードバックコマンドは`終了待ち`フラグやユーザ定義波形の長さ，キャプチャパラメータの値によって実行時間が変わります．
ユーザ定義波形とキャプチャパラメータの詳細は，[e7awg_hw ユーザマニュアル](https://github.com/e-trees/e7awg_sw/tree/main/manuals) の 3, 4 章を参照してください．
下図の `AWG スタート時刻`はコマンドの`スタート時刻`フィールドで指定した時刻です．
...
t_1 &= \left\{
\begin{array}{ll}
  t_0 + 568 & (スタート時刻フィールドのビットが全て 1 の場合)\\
  8b & (その他)\\
\end{array} \\
\right. \\[2ex]
b &= スタート時刻フィールドの値 \\[4ex]
...
&\hspace{-1.4em} キャプチャユニット n \;(0 \le n \le 9) \;のキャプチャパラメータから求まる値を V_n と置いたとき \\[1ex]

B_2 &= max \:\{V_u \;|\; u \in U\} \;\;\;\;\; (可変) \\[1ex]
V_u &= \left\{
\begin{array}{ll}
  2D_u + 1272 \;\; & (パターン 1) \\[1ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{min\{4, \;M_uN_u\}} S_u(i) + \displaystyle\sum_{i=1}^{min\{3, \;M_uN_u\}} P_u(i) + D_u \right\rparen + 1280 & (パターン 2) \\[4ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{M_u(N_u-1)} \left\{S_u(i) +  P_u(i) \right\} + D_u \right\rparen + 1272 & (パターン 3) \\[4ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{M_uN_u} \left\{S_u(i) +  P_u(i) \right\} + D_u \right\rparen + 1280 & (パターン 4) \\[4ex]
  2 \left\lparen\displaystyle\sum_{i=1}^{M_u(N_u-1)+4} S_u(i) + \displaystyle\sum_{i=1}^{M_u(N_u-1)+3} P_u(i) + D_u \right\rparen + 1272 & (パターン 5) \\
\end{array} \\
\right.\\
\\[1ex]
...
本項では，四値チャネルは外部四値チャネルが使用されることを想定し，外部 AWG スタートトリガと四値が同時に入力される場合の実行時間を示します．
四値付き外部トリガ待ち AWG スタートコマンドは `終了待ち` フラグを有効にするかどうかで実行時間が変わります．
下図の `外部 AWG スタートトリガと四値の入力` 時刻は，フィードバックシステムの `sequencer` モジュールに `外部 AWG スタートトリガ` と `四値` が入力された時刻です．

**終了待ちが無効の場合**

![四値付き外部トリガ待ちAWGスタートコマンドタイムチャート0](./feedback_figures/cmd_time/awg_start_with_ext_trig_and_four_cls_cmd_0.png)

**終了待ちが有効の場合**

![四値付き外部トリガ待ちAWGスタートコマンドタイムチャート1](./feedback_figures/cmd_time/awg_start_with_ext_trig_and_four_cls_cmd_1.png)

![高四値付き外部トリガ待ちAWGスタートコマンドタイムチャート式](./feedback_figures/cmd_time/awg_start_with_ext_trig_and_four_cls_cmd_eq.png)
...
A &= \left\{
\begin{array}{ll}
  0 & (波形パラメータの設定準備完了前に外部 AWG スタートトリガと四値が入力された場合)\\
  不定 & (その他) \;\; \\
\end{array} \\
\right. \\[3ex]
...
本項では，1 回目の波形出力に合わせてスタートしたキャプチャユニットが算出した四値を元に，2 回目に出力される波形が決まる場合の実行時間を示します．
条件付きフィードバックコマンドは`終了待ち`フラグやユーザ定義波形の長さ，キャプチャパラメータの値によって実行時間が変わります．
ユーザ定義波形とキャプチャパラメータの詳細は，[e7awg_hw ユーザマニュアル](https://github.com/e-trees/e7awg_sw/tree/main/manuals) の 3, 4 章を参照してください．
下図の `AWG スタート時刻`はコマンドの`スタート時刻`フィールドで指定した時刻です．

**終了待ちが無効の場合**

![条件付きFBコマンドタイムチャート0](./feedback_figures/cmd_time/conditional_feedback_cmd_0.png)

**終了待ちが有効の場合**

![条件付きFBコマンドタイムチャート1](./feedback_figures/cmd_time/conditional_feedback_cmd_1.png)

![条件付きFBコマンドタイムチャート式](./feedback_figures/cmd_time/conditional_feedback_eq.png)
...
V_u &= \left\{
\begin{array}{ll}
  2D_u + 1296 \;\; & (パターン 1) \\[1ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{min\{4, \;M_uN_u\}} S_u(i) + \displaystyle\sum_{i=1}^{min\{3, \;M_uN_u\}} P_u(i) + D_u \right\rparen + 1304 & (パターン 2) \\[4ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{M_u(N_u-1)} \left\{S_u(i) +  P_u(i) \right\} + D_u \right\rparen + 1296 & (パターン 3) \\[4ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{M_uN_u} \left\{S_u(i) +  P_u(i) \right\} + D_u \right\rparen + 1304 & (パターン 4) \\[4ex]
  2 \left\lparen\displaystyle\sum_{i=1}^{M_u(N_u-1)+4} S_u(i) + \displaystyle\sum_{i=1}^{M_u(N_u-1)+3} P_u(i) + D_u \right\rparen + 1296 & (パターン 5) \\
\end{array} \\
\right.\\
\\[1ex]
...
- (k) 四値付き外部トリガ待ち AWG スタートコマンドエラーレポート
- (l) 条件付きフィードバックコマンドエラーレポート
...
| ---- | ---- |
| レポート ID | 0x01 |
| AWG ID リスト |AWG スタートコマンドの「スタート時刻」フィールド (以降 A と呼ぶ) の値によって意味が異なります．<br><br>【A のビットに 0 が含まれる場合】<br> AWG スタートコマンドで指定した AWG のうち，指定した時刻にスタートできなかったもののリストです．<br> N bit 目が AWG N に対応しています．<br> 同コマンドがスタート時刻より前に中止された場合，このフィールドは 0 となります． <br><br>【A のビットが全て 1 であった場合】<br> AWG スタートコマンドで指定した AWG のうち，コマンドの実行開始時に動作中であったもののリストです．<br> N bit 目が AWG N に対応しています． |
...
| ---- | ---- |
| レポート ID | 0x08 |
| AWG ID リスト |高速フィードバックコマンドの「スタート時刻」フィールド (以降 A と呼ぶ) の値によって意味が異なります．<br><br>【A のビットに 0 が含まれる場合】<br> 高速フィードバックコマンドで指定した AWG のうち，指定した時刻にスタートできなかったもののリストです．<br> N bit 目が AWG N に対応しています．<br> 同コマンドがスタート時刻より前に中止された場合，このフィールドは 0 となります． <br><br>【A のビットが全て 1 であった場合】<br> 高速フィードバックコマンドで指定した AWG のうち，コマンドの実行開始時に動作中であったもののリストです．<br> N bit 目が AWG N に対応しています． |
| リードエラー | 波形パラメータを HBM から読みだすときにエラーが発生すると 1 になります． |
| ライトエラー | 波形パラメータを AWG に書き込むときにエラーが発生すると 1 になります． |
...
四値付き外部トリガ待ち AWG スタートコマンドがエラーを起こしたときに発行されます．

![四値付き外部トリガ待ち AWG スタートコマンドエラー](./feedback_figures/awg_start_with_ext_trig_and_cls_val_cmd_err.png)

| フィールド名 | 説明 |
| ---- | ---- |
| レポート ID | 0x0B |
| AWG ID リスト | 四値付き外部トリガ待ち AWG スタートコマンドで指定した AWG の内，スタートできなかったもののリストです．同コマンドがタイムアウトにより失敗した場合，指定した全ての AWG のビットが立ちます．加えて，AWG の波形出力準備開始時に，それが動作中であった場合も対応するビットが立ちます．<br> M bit 目が AWG N に対応しています． |
| リードエラー | 波形パラメータを HBM から読みだすときにエラーが発生すると 1 になります．|
| ライトエラー | 波形パラメータを HBM に書き込むときにエラーが発生すると 1 になります．|
| タイムアウト | 四値付き外部トリガ待ち AWG スタートコマンドの処理が始まってから，同コマンドの `タイムアウト` フィールドで指定した時間が過ぎるまでに，外部 AWG スタートトリガが入力されなかった場合 1 になります． |
...
条件付きフィードバックコマンドがエラーを起こしたときに発行されます．

![条件付きフィードバックコマンドエラー](./feedback_figures/conditional_feedback_cmd_err.png)

| フィールド名 | 説明 |
| ---- | ---- |
| レポート ID | 0x0C |
| AWG ID リスト | 条件付きフィードバックコマンドの「スタート時刻」フィールド (以降 A と呼ぶ) の値によって意味が異なります．<br><br>【A のビットに 0 が含まれる場合】<br> 条件付きフィードバックコマンドで指定した AWG のうち，指定した時刻にスタートできなかったもののリストです．<br> N bit 目が AWG N に対応しています．<br> 同コマンドがスタート時刻より前に中止された場合，このフィールドは 0 となります． <br><br>【A のビットが全て 1 であった場合】<br> 条件付きフィードバックコマンドで指定した AWG のうち，コマンドの実行開始時に動作中であったもののリストです．<br> N bit 目が AWG N に対応しています． |
| リードエラー | 波形パラメータを HBM から読みだすときにエラーが発生すると 1 になります． |
| ライトエラー | 波形パラメータを AWG に書き込むときにエラーが発生すると 1 になります． |
