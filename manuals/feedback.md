...
`高速フィードバックコマンド` と `四値付き外部トリガ待ち AWG スタートコマンド` で AWG が出力する波形の候補を指定するコマンドです．
...
このコマンドは，その際の **AWG**，**四値チャネル**，**波形パラメータブロック** の組み合わせを指定します．

`高速フィードバックコマンド` と `四値付き外部トリガ待ち AWG スタートコマンド` で動作する全ての AWG に対して，事前に本コマンドで四値チャネルと波形パラメータブロックを指定する必要があります．
...
V_u &= \left\{
\begin{array}{ll}
  2D_u + 1256 \;\; & (パターン 1) \\[1ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{min\{4, \;M_uN_u\}} S_u(i) + \displaystyle\sum_{i=1}^{min\{3, \;M_uN_u\}} P_u(i) + D_u \right\rparen + 1264 & (パターン 2) \\[4ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{M_u(N_u-1)} \left\{S_u(i) +  P_u(i) \right\} + D_u \right\rparen + 1256 & (パターン 3) \\[4ex]
  2 \left\lparen \displaystyle\sum_{i=1}^{M_uN_u} \left\{S_u(i) +  P_u(i) \right\} + D_u \right\rparen + 1264 & (パターン 4) \\[4ex]
  2 \left\lparen\displaystyle\sum_{i=1}^{M_u(N_u-1)+4} S_u(i) + \displaystyle\sum_{i=1}^{M_u(N_u-1)+3} P_u(i) + D_u \right\rparen + 1264 & (パターン 5) \\
\end{array} \\
\right.\\
\\[1ex]
