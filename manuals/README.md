...
| capture module | 1つ以上の capture unit を束ねるモジュールです．同じ capture module の中の capture unit は全て同じ入力波形データを受け取ります．|
| capture unit 0 ~ 7 | 入力波形データに信号処理を適用して HBM に格納します．|
| capture unit 8 ~ 9 | 入力波形データの値を変えずに HBM に格納します．|
...
ユーザは，各キャプチャユニットに対し**キャプチャ区間**を定義することで，入力波形のどの部分に信号処理を適用するかを決めることができます．
...
**総和区間**は最大 4096 個まで定義でき，個別に長さを設定できます．
...
キャプチャユニット 0 ~ 7 には信号処理機能がありますが，8 ~ 9 にはありません．
キャプチャユニット 0 ~ 7 は次項で説明する信号処理の結果を出力するのに対し，キャプチャユニット 8 ~ 9 は**総和区間**に含まれる入力データを値を変えずに出力します．
どちらの出力データも，**4.7 HBM に格納されたキャプチャデータの並び** に示すフォーマットで HBM に格納されます．

|  キャプチャユニット | 入力データ | 出力データ | 信号処理機能 |
| ---- | ---- | ---- | ---- |
| 0 ~ 7   | I/Q データ | I/Q データ or 四値化結果 (※1) | あり |
| 8 ~ 9   | I/Q データ | I/Q データ | なし |

※1 適用する信号処理の設定による

#### 4.3.2 信号処理詳細

キャプチャユニット 0 ~ 7 には以下の信号処理回路が含まれます．
...
信号処理の有効 / 無効を設定するには，**4.4 キャプチャ制御レジスタ一覧** の `信号処理有効/無効` レジスタと `信号処理モジュール有効/無効` レジスタを使用します．
`信号処理有効/無効` レジスタの値が 1 のとき `信号処理モジュール有効/無効` で個別の処理の有効/無効を選択することができます．

`信号処理有効/無効` レジスタの値を 0 にして信号処理機構を無効にした場合と, これを有効にしてかつ `信号処理モジュール有効/無効` レジスタで全ての信号処理モジュールを無効にした場合のキャプチャユニットの動作は同じです．
ただし，キャプチャユニットに入力された波形データが出力されるまでにかかる時間（以下キャプチャレイテンシと呼ぶ）は異なります．
信号処理機構を無効にした場合，キャプチャユニット 0 ~ 7 とキャプチャユニット 8 ~ 9 のキャプチャレイテンシは，クロックドメイン間のデータ同期によるズレを除いて同じになります．
...
### 4.6 キャプチャ制御パケットフォーマット

**4.4 キャプチャ制御レジスタ一覧**のレジスタにアクセスするためには，キャプチャ制御パケットを UDP データとして e7awg_hw のポート 16385 に送る必要があります．
