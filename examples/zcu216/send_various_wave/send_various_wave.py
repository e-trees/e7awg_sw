...
WAVE_FREQ_0 = 4 # MHz
WAVE_FREQ_1 = 1.2 # MHz  (波形チャンクに波形パートを追加するときに 0 パディングが必要ない周波数)
...
def gen_wave_samples(waveform, amplitude, freq, num_cycles, sampling_rate, hw_specs):
    """引数に応じて「正弦波」「ノコギリ波」「矩形波」のいずれかを作る"""
    if waveform == 'sin':
        wave = e7s.SinWave(num_cycles, freq, amplitude)
    elif waveform == 'cos':
        wave = e7s.SinWave(num_cycles, freq, amplitude, phase = math.pi / 2)
    elif waveform == 'saw':
        wave = e7s.SawtoothWave(
            num_cycles, freq, amplitude, phase = math.pi, crest_pos = 0.0)
    elif waveform == 'squ':
        wave = e7s.SquareWave(num_cycles, freq, amplitude)
    elif waveform == 'const':
        wave = e7s.SquareWave(num_cycles, freq, amplitude, duty_cycle = 1)

    # DAC の I/Q ミキサが有効な場合, RF Data Converter 内部で波形データが補間されるので, あらかじめ 1/4 に間引いておく.
    samples = wave.gen_samples(sampling_rate)[0::4]
    ...
    for awg_id, params in awg_to_params.items():
        ...
        for waveform, num_cycles in params.chunk_waves:
            samples = gen_wave_samples(
                ...
                sampling_rate,
                hw_specs)
            wave_seq.add_chunk(samples, params.num_blank_words, params.num_chunk_repeats)
...
def setup_awgs(awg_ctrl, sampling_rate, hw_specs, awg_list):
    """AWG の波形出力に必要な設定を行う"""
    # AWG 初期化
...
def main(design_type, awg_list):
    ...
    with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as transceiver,
        ...
        awg_to_wave_seq = setup_awgs(awg_ctrl, sampling_rate, hw_specs, awg_list)
        # ディジタル出力モジュールのセットアップ
...
if __name__ == "__main__":
    ...
    awg_list = sorted(e7s.AWG.on(e7s.E7AwgHwType.ZCU216))
    if args.awgs is not None:
        awg_list = [e7s.AWG(int(x)) for x in args.awgs.split(',')]

    main(e7s.E7AwgHwType.ZCU216, awg_list)
