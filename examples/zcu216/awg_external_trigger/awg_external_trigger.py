...
def set_wave_sequence(awg_list, awg_ctrl, design_type, hw_specs):
    awg_to_wave_seq = {}
...
def setup_awgs(awg_list, awg_ctrl, design_type, hw_specs):
    """AWG の波形出力に必要な設定を行う"""
    # AWG 初期化
    awg_ctrl.initialize(*awg_list)
    # 波形データを AWG に設定
    return set_wave_sequence(awg_list, awg_ctrl, design_type, hw_specs)


def set_digital_out_data(digital_out_ctrl, design_type):
    # ディジタル出力データの作成
    dout_data_list = e7s.DigitalOutputDataList(design_type)
    output_time = 307 # 1 [usec]
    dout_data_list.add(1, output_time)
...
def main(design_type, awg_list):
    ...
    with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as transceiver,
        ...
        # AWG のセットアップ
        print('setup AWGs')
        setup_awgs(awg_list, awg_ctrl, design_type, hw_specs)
        # ディジタル出力モジュールのセットアップ
