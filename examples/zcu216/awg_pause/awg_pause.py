...
def set_waves(awg_list, awg_ctrl, sampling_rate, hw_specs):
    ...
    for awg_id in awg_list:
        ...
        num_repeats = int(5 * sampling_rate / (len(samples) * 4) ) # 5 秒分の繰り返し回数
...
def setup_digital_output_modules(digital_out_ctrl, design_type):
    ...
    # AWG からのスタートトリガ, 一時停止トリガ, 再開トリガを受け付けるように設定.
    triggers = [
        e7s.DigitalOutTrigger.START,
        e7s.DigitalOutTrigger.PAUSE,
        e7s.DigitalOutTrigger.RESUME ]
    digital_out_ctrl.enable_trigger(triggers, e7s.DigitalOut.U0)
...
def main(design_type, awg_list):
    ...
    with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as transceiver,
        ...
        time.sleep(2)
        # 波形出力一時停止
        awg_ctrl.pause_awgs(*awg_list)
        input("\nPress 'Enter' to resume AWGs\n")
        # 波形出力再開
        print('resume AWGs')
        awg_ctrl.resume_awgs(*awg_list)
        # 波形出力完了待ち
        print('wait for AWGs to stop')
        awg_ctrl.wait_for_awgs_to_stop(7, *awg_list)
        # ディジタル出力モジュール動作完了待ち
