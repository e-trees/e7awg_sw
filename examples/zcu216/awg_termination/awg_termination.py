...
def set_wave_sequence(awg_ctrl, awg_list, hw_specs):
    awg_to_wave_seq = {}
    for awg_id in awg_list:
        wave_seq = e7s.WaveSequence(0, 0xFFFF_FFFF, hw_specs.design_type)
        samples = gen_wave_samples(wave_freq * 1e6, 4, hw_specs)
...
def setup_awgs(awg_ctrl, awg_list, hw_specs):
    """AWG の波形出力に必要な設定を行う"""
    # AWG 初期化
    awg_ctrl.initialize(*awg_list)
    # 波形データを AWG に設定
    return set_wave_sequence(awg_ctrl, awg_list, hw_specs)
...
def main(design_type, awg_list):
    ...
    with (e7sz.RftoolTransceiver(zcu216_ip_addr, 15) as transceiver,
        ...
        # AWG のセットアップ
        print('setup AWGs')
        setup_awgs(awg_ctrl, awg_list, hw_specs)
        # ディジタル出力モジュールのセットアップ
