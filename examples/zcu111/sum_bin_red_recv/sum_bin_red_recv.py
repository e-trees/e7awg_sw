...
DspParam = namedtuple('DspParam', ['delay', 'sum_len', 'bin_threshold', 'reduction_op'])
...
def main(
    ...
    with (e7sz.RftoolTransceiver(ip_addr.zcu111, 15) as transceiver,
        ...
        expected = calc_expected_cap_data(
            [bc.DspUnit.SUM, bc.DspUnit.BINARIZATION, bc.DspUnit.REDUCTION],
            cap_unit_to_cap_param[dsp_cap_unit],
            cap_unit_to_cap_data[raw_cap_unit],
            hw_specs)
        # 生データにソフトウェアで信号処理を適用
        sum_bin_result = calc_expected_cap_data(
            [bc.DspUnit.SUM, bc.DspUnit.BINARIZATION],
            cap_unit_to_cap_param[dsp_cap_unit],
            cap_unit_to_cap_data[raw_cap_unit],
            hw_specs)
        sum_result = calc_expected_cap_data(
            [bc.DspUnit.SUM],
            cap_unit_to_cap_param[dsp_cap_unit],
            cap_unit_to_cap_data[raw_cap_unit],
            hw_specs)
...
if __name__ == "__main__":
    ...
    if design_type == e7s.E7AwgHwType.ZCU111_URAM_X2:
        ...
        bin_threshold = int(-2e5) if reduction_op == bc.ReductionOperation.ALL else int(1.25e5)
    ...
    cap_unit_to_dsp_param = {
        e7s.CaptureUnit.U0: DspParam(capture_delay, sum_len, bin_threshold, reduction_op),
        e7s.CaptureUnit.U1: DspParam(capture_delay, None, None, None)
    }
