...
option_to_wave_params = {

    # const  const  const _  const  const  const _  (出力パターン)
    0 : wave_params(
        freq = WAVE_FREQ_0,
        num_wait_words = 0,
        num_seq_repeats = 1,
    ...
    # const  const  const  _  const  const  const _
    1 : wave_params(
        freq = WAVE_FREQ_0,
        num_wait_words = 0,
        num_seq_repeats = 1,
...
def set_wave_sequence(awg_ctrl, hw_specs, wave_option):
    awg_to_wave_seq = {}
...
def setup_awgs(awg_ctrl, hw_specs, wave_option):
    """AWG の波形出力に必要な設定を行う"""
    # AWG 初期化
    awg_ctrl.initialize(*awg_list)
    # 波形データを AWG に設定
    return set_wave_sequence(awg_ctrl, hw_specs, wave_option)


def setup_dacs(rfdc_ctrl, mixer_option):
    """DAC の設定を行う"""
    dac_tiles = [e7sz.DacTile.T0, e7sz.DacTile.T1]
    dac_channels = [e7sz.DacChannel.C0, e7sz.DacChannel.C1, e7sz.DacChannel.C2, e7sz.DacChannel.C3]
    settings = option_to_mixer_settings[mixer_option]
    for tile in dac_tiles:
        ...
        for channel in dac_channels:
            ...
            rfdc_ctrl.set_dac_mixer_settings(
                tile, channel, settings.freq, settings.phase_offset, settings.amplitude)
            # DAC 割り込みクリア
...
def main(design_type, waveform):
    ...
    with (e7sz.RftoolTransceiver(zcu111_ip_addr, 15) as transceiver,
        ...
        # AWG のセットアップ
        print('setup AWGs')
        awg_to_wave_seq = setup_awgs(awg_ctrl, hw_specs, waveform)
        # ディジタル出力モジュールのセットアップ
