...
# sum_len: 総和区間の長さ (秒)
# bin_threshold: 二値化閾値
# reduction_op: リダクション処理の種類
DspParam = namedtuple(
    'DspParam', 
    ['delay', 'enable_decimation', 'sum_len', 'bin_threshold', 'reduction_op'])
...
def set_capture_params(cap_ctrl, cap_unit, dsp_param, wave_sequence, cap_unit_specs):
    ...
    if dsp_param.bin_threshold is not None:
        dsp_list.append(bc.DspUnit.BINARIZATION)
        cap_param.real_bin_threshold = dsp_param.bin_threshold
    if dsp_param.reduction_op is not None:
        dsp_list.append(bc.DspUnit.REDUCTION)
        cap_param.real_reduction_op = dsp_param.reduction_op
...
def output_capture_waveform(cap_unit, samples, title, file_name, sampling_rate):
    dirpath = 'plot_capture_data/Capture_{}/'.format(cap_unit)
    os.makedirs(dirpath, exist_ok=True)
    e7s.plot_graph(
        sampling_rate,
...
def output_results(
    ...
    output_capture_waveform(raw_cap_unit, dec_result, 'dec (SW)', 'sw_dec.png', cap_sampling_rate)
    output_capture_waveform(raw_cap_unit, raw_data, 'raw', 'raw.png', cap_sampling_rate)
...
def main(
    ...
    with (e7sz.RftoolTransceiver(ip_addr.zcu111, 15) as transceiver,
        ...
        expected = calc_expected_cap_data(
            [bc.DspUnit.DECIMATION, bc.DspUnit.SUM, bc.DspUnit.BINARIZATION, bc.DspUnit.REDUCTION],
            cap_unit_to_cap_param[dsp_cap_unit],
            cap_unit_to_cap_data[raw_cap_unit],
            hw_specs)
        dec_sum_bin_result = calc_expected_cap_data(
            [bc.DspUnit.DECIMATION, bc.DspUnit.SUM, bc.DspUnit.BINARIZATION],
            cap_unit_to_cap_param[dsp_cap_unit],
            cap_unit_to_cap_data[raw_cap_unit],
            hw_specs)
        dec_sum_result = calc_expected_cap_data(
            [bc.DspUnit.DECIMATION, bc.DspUnit.SUM],
            cap_unit_to_cap_param[dsp_cap_unit],
            cap_unit_to_cap_data[raw_cap_unit],
            hw_specs)
        dec_result = calc_expected_cap_data(
        ...
        output_results(
            ...
            cap_unit_to_cap_param[dsp_cap_unit].real_bin_threshold)
        print('end')
...
def get_program_args():
    ...
    parser.add_argument('--capture-delay', default=6.5e-8, type=float) # second
...
if __name__ == "__main__":
    ...
    if design_type == e7s.E7AwgHwType.ZCU111_DOUBLE_QUANTUM_DOT:
        ...
        bin_threshold = int(-4e4) if reduction_op == bc.ReductionOperation.ALL else int(4e4)
        sum_len = 2.1e-8 # second
    ...
    cap_unit_to_dsp_param = {
        e7s.CaptureUnit.U0: DspParam(args.capture_delay, True, sum_len, bin_threshold, reduction_op),
        e7s.CaptureUnit.U1: DspParam(args.capture_delay, False, None, None, None)
    }
