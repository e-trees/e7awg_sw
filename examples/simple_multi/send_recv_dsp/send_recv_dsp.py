...
def gen_cos_wave(freq, num_cycles, amp, hw_specs):
    ...
    freq : MHz
    """
    dt = 2.0 * math.pi * (freq * 1e6) / hw_specs.awg.sampling_rate
    num_samples = int(num_cycles * hw_specs.awg.sampling_rate / (freq * 1e6))
    i_data =  [int(amp * math.cos(i * dt)) for i in range(num_samples)]
    q_data =  [int(amp * math.sin(i * dt)) for i in range(num_samples)]
    return IqWave.convert_to_iq_format(i_data, q_data, hw_specs.awg.smallest_unit_of_wave_len)
...
def gen_wave_seq(hw_specs):
    ...
    num_chunks = 1
    samples = gen_cos_wave(5.0, NUM_WAVE_CYCLES, 32760, hw_specs)
