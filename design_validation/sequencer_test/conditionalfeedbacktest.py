...
from e7awgsw import \
    WaveSequenceSetCmd, CaptureParamSetCmd, \
    CaptureAddrSetCmd, WaveSequenceSelectionCmd, \
    ConditionalFeedbackCmd, FourClassifierChannel
...
# このサンプル値を四値化した結果をもとに, 条件付きフィードバック命令の2回目の波形を選択する
SAMPLE_FOR_FOUR_CLS_0 = (2, 1)
SAMPLE_FOR_FOUR_CLS_1 = (-2, -1) 

class ConditionalFeedbackTest(object):
    ...
    __CAP_MOD_TO_UNITS = {
        CaptureModule.U3 : [CaptureUnit.U0, CaptureUnit.U4],
        CaptureModule.U1 : [CaptureUnit.U6, CaptureUnit.U3],
    }
    ...
    def __init__(self, res_dir, awg_cap_ip_addr, seq_ip_addr, server_ip_addr, use_labrad):
        ...
        self.__awgs = [AWG.U4, AWG.U15]

        # 条件付きフィードバック処理で使用する四値を算出するキャプチャユニット
        self.__cap_units_with_cls = [
            self.__CAP_MOD_TO_UNITS[CaptureModule.U3][0], # U0
            self.__CAP_MOD_TO_UNITS[CaptureModule.U1][0]  # U6
        ]
        # AWG の出力波形を検証するために使用するキャプチャユニット
        self.__cap_units_plain = [
            self.__CAP_MOD_TO_UNITS[CaptureModule.U3][1], # U4
            self.__CAP_MOD_TO_UNITS[CaptureModule.U1][1]  # U3
        ]
    ...
    def __gen_cmds(
        ...
        cmds = [
            ...
            ConditionalFeedbackCmd(7, self.__awgs, time, feedback_flag, wait = True, stop_seq = True)
        ]
        return cmds
    ...
    def __gen_expected_data(
        ...
        second_wave_seqs_1,
        four_cls_val_1):
        exp_data_0 = first_wave_seq_0.all_samples(False)
        if feedback_flag == four_cls_val_0:
            exp_data_0 += second_wave_seqs_0[four_cls_val_0].all_samples(False)
        exp_data_1 = first_wave_seq_1.all_samples(False)
        if feedback_flag == four_cls_val_1:
    ...
    def __save_wave_data(
        ...
        self.__save_wave_samples(
            cap_unit_to_cap_data, test_name, 'cond_fb_{}_captured'.format(test_no))
        self.__save_wave_samples(
            cap_unit_to_exp_cap_data, test_name, 'cond_fb_{}_expected'.format(test_no))
    ...
    def __compare_cap_data(self, cap_units, data_0, data_1, test_no):
        ...
        for cap_unit, wave_0, wave_1 in list(zip(cap_units, data_0, data_1)):
            if wave_0 != wave_1:
                all_match = False
                print('cond fb {}, cap_unit {} error'.format(test_no, cap_unit))
        return all_match
    ...
    def run_test(self, test_name):
        """
        テスト項目
        ・条件付きフィードバック処理で AWG が出力する波形が, 四値化結果に応じて適切に切り替わる
        
          +--------------------------------------------------------------+
          |                     ___________                              |
          |                +-> | capture 4 |     _____________________   |
          |                |   +-----------+    |      Sequencer      |  |
          |     ________   |    ___________     |                     |  |
          +--> | AWG 4  | -+-> | capture 0 | -> |-four cls ch 0       | -+
               +--------+      +-----------+    |                     |
                ________        ___________     |                     |
          +--> | AWG 15 | -+-> | capture 6 | -> |-four cls ch 6       | -+
          |    +--------+  |   +-----------+    +---------------------+  |
          |                |    ___________                              |
          |                +-> | capture 3 |                             |
          |                    +-----------+                             |
        ...
        条件付きフィードバック処理で 2 回出力される波形を, 同処理中に 1 回実行されるキャプチャ (DSP は全て無効) でまとめて取得する.
        このとき, [1回目の波形]-[0 データ]-[2回目の波形]-[0データ] という順番で波形がキャプチャされる.
        2回目の波形は, 四値に対応する波形シーケンスか 0 データのどちらかとなる.
        よって, このキャプチャデータから 0 データを取り除くと [1回目の波形]-[2回目の波形] か [1回目の波形] のどちらかとなる.
        ソフトウェアであらかじめ計算しておいた四値化結果をもとにどちらのパターンとなるかを決定し, 
        AWG から出力される波形と 0 データを取り除いたキャプチャデータが一致するかを確認する.
        """
        ...
        # フィードバックフラグ
        feedback_flag = [0, 2, 1, 3]
        
        success = True
        # 条件付きフィードバック処理の中で, 二回目の波形が各四値化結果に対応していたか検査する
        for i in range(4):
            # コマンド作成
            ...
            self.__save_capture_params(
                cap_params[i], test_name, 'cond_fb_{}_cap_param.txt'.format(i))
            # 期待値データ算出
...
def gen_cls_capture_params():
    """
    条件付きフィードバック処理で使用する四値化結果を算出するためのキャプチャパラメータを生成する.
    """
    params = []
    for i in range(4):
        param = CaptureParam()
